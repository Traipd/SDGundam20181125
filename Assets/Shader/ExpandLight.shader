Shader "SDGundam/Effect/ExpandLight" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert(a2v v) {
				v2f o;
				float4 laterV=v.vertex+float4(v.normal,0)*_offset*0.15f;
		...
		Pass {
			...
			v2f vert(a2v v) {
				v2f o;
				float4 laterV=v.vertex+float4(v.normal,0)*_offset*0.08f;
				o.pos= UnityObjectToClipPos(laterV);
				o.uv = TRANSFORM_TEX(v.texcoord, _MainTex);	
				o.worldNormal = UnityObjectToWorldNormal(v.normal);
				o.worldPos = mul(unity_ObjectToWorld, laterV).xyz;				
		...
		Pass {
			...
			v2f vert(a2v v) {
				v2f o;
				float4 laterV=v.vertex;//+float4(v.normal,0)*_offset
