...
public class TrailingMesh1_skin : MonoBehaviour {
	  ...
	  {	
		...
		#region-------从SkinnedMeshRenderer上拷贝可以正确显示的网格
			...
			for(int j=count-1;j>=1;j--)//刷新顶点位置
			{
                ...
                for(int i=Fmesh.vertices.Length-1;i>=0;i--)
				{
					v3s[i]=FgameObject.GetComponent<Transform>().localToWorldMatrix.MultiplyPoint(Fmesh.vertices[i]);//+Vector3.up*(1)
				}
