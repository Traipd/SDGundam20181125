...
public class TrailingMesh2 : MonoBehaviour {//此版本为SkinnedMeshRenderer和MeshRenderer共用版本
 	public bool canSeeAtStart=true;
	// public bool use_SkinnedMeshRenderer_flag=false;
	...
	void Start () {
		...
		{
			Srender = GetComponent<SkinnedMeshRenderer>();
		}
		...
		for(int j=0;j<count;j++)//设置uv和顶点绘制顺序
			{
				for(int i=0;i<Fmesh.vertices.Length;i++)
				{
					if(Srender!=null)
					{
						v3s[j*Fmesh.vertices.Length+i]=FgameObject.GetComponent<Transform>().localToWorldMatrix.MultiplyPoint(Fmesh.vertices[i]);	
					}else
					{
					...
					if((i<Fmesh.vertices.Length-1)&&(j<count-1))
					{
						...
						t2s[(j*(Fmesh.vertices.Length-1)+i)*6+3]=j*Fmesh.vertices.Length+i;
						t2s[(j*(Fmesh.vertices.Length-1)+i)*6+4]=j*Fmesh.vertices.Length+i+1;
						t2s[(j*(Fmesh.vertices.Length-1)+i)*6+5]=(j+1)*Fmesh.vertices.Length+i+1;
					}	  			
		...
		#region-------从SkinnedMeshRenderer上拷贝可以正确显示的网格
			...
			for(int j=count-1;j>=1;j--)//刷新顶点位置
			{
                ...
                for(int i=Fmesh.vertices.Length-1;i>=0;i--)
				{
					if(Srender!=null)
					{
						v3s[i]=FgameObject.GetComponent<Transform>().localToWorldMatrix.MultiplyPoint(Fmesh.vertices[i]);	
					}else
					{
						v3s[i]=GetComponent<Transform>().localToWorldMatrix.MultiplyPoint(Fmesh.vertices[i]);
					}
				}
